...
function lay_so_luong_san_pham_aosomi(){
    $sql = "SELECT COUNT(*) AS total FROM sanpham WHERE iddm = 3"; // Truy vấn để đếm tổng số sản phẩm
    $ket_qua = pdo_query_one($sql);
...
function lay_so_luong_san_pham_aophong(){
    $sql = "SELECT COUNT(*) AS total FROM sanpham WHERE iddm = 5"; // Truy vấn để đếm tổng số sản phẩm
    $ket_qua = pdo_query_one($sql);
